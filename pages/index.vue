...
export default {
  ...
  methods: {
    async getMovies() {
      ...
      data.results.forEach(movie => this.movies.push(movie));
    },
    async searchMovies() {
      const { data } = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=17735fbb9638eca205193ecd3361df71&language=en-US&page=1&query=${this.searchInput}`);
      data.results.forEach(movie => this.searchedMovies.push(movie));
    },
    clearSearch() {
      this.searchInput = '';
      this.searchedMovies = [];
